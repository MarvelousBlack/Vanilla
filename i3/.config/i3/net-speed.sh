...
getmemusage="awk '/MemTotal/ {memtotal=\$2}; /MemAvailable/ {memavail=\$2}; END { printf(\"%.0f\", (100- ((memavail / memtotal )* 100))) }' /proc/meminfo"
i3status | (read line && echo "$line" && read line && echo "$line" && read line && echo "$line" && update_rate && while :
do
  read line
  update_rate
  echo ",[{\"full_text\":\"Speed:${rate}\" },{\"full_text\":\"Memory:$(eval $getmemusage)%\" },${line#,\[}" || exit 1
done)
